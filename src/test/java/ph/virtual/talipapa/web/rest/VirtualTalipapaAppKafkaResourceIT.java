...
import ph.virtual.talipapa.config.KafkaProperties;

class VirtualTalipapaAppKafkaResourceIT {
    ...
    void setup() {
        ...
        VirtualTalipapaAppKafkaResource kafkaResource = new VirtualTalipapaAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/virtual-talipapa-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/virtual-talipapa-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
